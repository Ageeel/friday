...
def get_notifications_view(page: ft.Page):
    ...
    return ft.View("/notifications", bgcolor=BG_COLOR, padding=20, controls=[
        ft.Container(content=ft.Row([
            ft.Column([ft.Text("إجمالي الودائع", color=ACCENT_GREEN, size=12), ft.Text(f"{total_deposit:,.0f}", color=ACCENT_GREEN, size=20, weight=ft.FontWeight.BOLD)], expand=True, horizontal_alignment=ft.CrossAxisAlignment.CENTER),
            ft.VerticalDivider(width=1, color="#333333"),
            ft.Column([ft.Text("إجمالي السحب", color="#c94559", size=12), ft.Text(f"{total_withdraw:,.0f}", color="#c94559", size=20, weight=ft.FontWeight.BOLD)], expand=True, horizontal_alignment=ft.CrossAxisAlignment.CENTER),
        ]), padding=20, bgcolor=CARD_COLOR, border_radius=20, border=ft.border.all(1, "#333333"),margin=ft.margin.only(top=30)
        ),
        notif_list
...
def build_notification_icon(page):
    ...
    return ft.Stack([
        ft.IconButton(icon=ft.icons.NOTIFICATIONS, on_click=lambda _: page.go("/notifications")),
