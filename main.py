...
def main(page: ft.Page):
    ...
    is_offline = False  # مؤشر لحالة اتصال الإنترنت
    view_mode = "active"
        ...
        if data:
            ...
            for key, val in data.items():
                ...
                if view_mode == "retracted":
                    ...
                    # إذا انقطع الإنترنت، يتم عرض الصورة الافتراضية user.png من ملفات الـ assets فوراً
                    if is_offline:
                        profile_img = "user.png"
                    else:
                        profile_img = val.get("img") or "user.png"
    ...
    def load_data(e=None):
        ...
        def fetch_data():
            nonlocal all_data, is_offline
            try:
                res = requests.get(DB_URL, timeout=5)
                if res.status_code == 200 and res.json():
                    all_data = res.json()
                    is_offline = False  # الإنترنت متصل
                    try:
                ...
                is_offline = True  # الإنترنت مقطوع
                if not all_data:
