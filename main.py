...
def main(page: ft.Page):
    ...
    def render_data(data, query=""):
        ...
        if data:
            for key, val in data.items():
                ...
                members_list.controls.append(ft.ListTile(leading=ft.CircleAvatar(content=ft.Icon(ft.icons.PERSON, color=ft.colors.ORANGE_400), bgcolor=ft.colors.GREY_900), title=ft.Text(val.get("name", "مجهول"), color=ft.colors.GREY_200), subtitle=ft.Text(status_text, color=ft.colors.GREY_500, size=13), trailing=ft.Icon(icon, color=icon_color)))
        
        tools_section.update_stats(total, paid, pending)
        total_balance_text.value = f"الرصيد {total_balance - total_withdrawals}" 
        total_retracted.value = f"المنسحبين {retracted_count}"
        page.update()
    ...
    def close_onboarding(e):
        ...
        refresh_button.visible = True # إظهار الزر عند الانتهاء
        page.update()
        load_data()
    ...
    onboarding_container = ft.Container(
        ...
        content=ft.Column(
            ...
            controls=[
                ...
                ft.Text("نحول مدخراتنا الأسبوعية إلى فرصة استثمارية حقيقية. يلا إشترك معانا والتزم بـ 1,000 جنيه كل يوم جمعة، كل دا عشان نسوي راس مال ، ونخطط لمشروع يجمعنا سوا في المستقبل، ونرفع مكانة الاسرة", size=20, color="white", text_align=ft.TextAlign.CENTER), 
                ft.ElevatedButton("ابدأ",on_click=close_onboarding, style=ft.ButtonStyle(bgcolor=ft.colors.ORANGE_600,color=ft.colors.BLACK
    ))]))

    def route_change(route):
        page.views.clear()
        header = ft.Container(bgcolor=ft.colors.GREY_900, padding=15, border_radius=10, content=ft.Row(alignment=ft.MainAxisAlignment.SPACE_BETWEEN, controls=[ft.Text(f"عدد الجمعات {total_fridays}", size=14, color=ft.colors.GREY_400), total_balance_text, total_retracted, build_notification_icon(page)]))
    ...
    # --- التحقق من الـ Onboarding عند التشغيل ---
    if not page.client_storage.get("onboarding_seen"):
        onboarding_container.visible = True
    else:
        refresh_button.visible = True # إظهار الزر إذا كان الـ onboarding قد تم رؤيته سابقاً
        load_data()
