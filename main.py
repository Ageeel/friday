...
def get_fridays_count():
    start_date = datetime(2026, 6, 12) #real is at 19
    today = datetime.now()
...
subtitle= ft.Text(status_text, color=ft.colors.GREY_500, size=13), trailing=ft.Icon(ft.icons.CHECK_CIRCLE if val.get("status") == "دفع" else ft.icons.CANCEL, color=ft.colors.LIME_600 if val.get("status") == "دفع" else "#e56328")))
    ...
    def route_change(route):
        ...
        def notification_card(text, amount, type, date):
        	...
        	return ft.Container(bgcolor=ft.colors.GREY_900,
        	...
        	controls=[
        	ft.Row(
        	[ft.Icon(t_icon, size=15, color = t_color),
        	ft.Text(t_text, color=t_color, size=13),
        	], spacing=5),
            ...
            if os.path.exists(NOTIFICATIONS_FILE):
                with open(NOTIFICATIONS_FILE, "r", encoding="utf-8") as f:
                    ...
                    for item in data.get("notifications", []):
                        notif_list.controls.append(notification_card(item.get('noti'), item.get('amount'), item.get('type'), item.get('date')))
                        if item.get("type") == "d":
                        	total_deposit += item.get("amount")
                        elif item.get("type") == "w":
                        	total_withdraw += item.get("amount")
            page.views.append(ft.View("/notifications", bgcolor=ft.colors.BLACK, padding=ft.padding.only(top=5), controls=[
                ft.AppBar(
                	bgcolor=ft.colors.GREY_900,
                    title=ft.Text(f"رصيد الودائع {total_deposit} - رصيد السحب {total_withdraw}", size=13, color= GREY),
                    leading=ft.IconButton(icon=ft.icons.NOTIFICATIONS, icon_color = GREY)
                ),
                notif_list
