...
def main(page: ft.Page):
    ...
    def render_data(data, query=""):
        ...
        # --- تحقق مما إذا كانت القائمة فارغة بعد التصفية ---
        if len(members_list.controls) == 0:
            empty_msg = "لا يوجد منسحبين" if view_mode == "retracted" else "لا يوجد مشتركين"
            members_list.controls.append(
                ft.Container(
                    padding=20,
                    alignment=ft.alignment.center,
    ...
    page.go("/")  # الانتقال وبناء الصفحة أولاً

    # التحقق من Onboarding أو تحميل البيانات
