...
def main(page: ft.Page):
    ...
    # --- المكونات والإنشاء ---
    all_data = {}
    view_mode = "active"  # "active" للمشتركين أو "retracted" للمنسحبين
    members_list = ft.ListView(expand=True, spacing=0)
    
    # تعريفات نصوص التوازن للعرض
    ...
    loading_overlay = ft.Container(visible=False, bgcolor=ft.colors.with_opacity(0.8, ft.colors.BLACK), alignment=ft.alignment.center, expand=True, content=ft.Column(alignment=ft.MainAxisAlignment.CENTER, horizontal_alignment=ft.CrossAxisAlignment.CENTER, controls=[ft.ProgressRing(), ft.Text("جار التحميل، إنتظر قليلا", color=ft.colors.WHITE, size=16, weight="bold")]))

    refresh_button = ft.FloatingActionButton(icon=ft.icons.REFRESH, bgcolor=ft.colors.ORANGE_600, on_click=lambda e: load_data(), width=40, height=40, visible=False)

    class Tools(ft.Container):
        def __init__(self, on_click_members=None):
            super().__init__()
            ...
            # تم إضافة دالة النقر عند التفاعل مع container المشتركين
            self.content = ft.Row(
                spacing=5, 
                controls=[
    ...
    tools_section = Tools(on_click_members=switch_to_active)
    ...
    def render_data(data, query=""):
        ...
        if data:
            for key, val in data.items():
                ...
                # حساب التكاليف والإحصاءات الأساسية للمشتركين النشطين
                if not is_retracted:
                    total += 1
                ...
                # فلترة حسب خيار العرض الحقيقي (نشط أم منسحب)
                if (view_mode == "active" and is_retracted) or (view_mode == "retracted" and not is_retracted):
                    continue

                if query and query not in val.get("name", ""): 
                    continue

                # بناء عناصر القائمة بناءً على النمط المفعل
                if view_mode == "retracted":
                    ...
                    members_list.controls.append(
                        ft.ListTile(
                            leading=ft.CircleAvatar(content=ft.Icon(ft.icons.PERSON, color=p_icon_col), bgcolor=ft.colors.GREY_900), 
...
if __name__ == "__main__":
    ft.app(target=main, view=ft.AppView.WEB_BROWSER, port=5050)
