...
def main(page: ft.Page):
    ...
    page.padding = 0 # تم ضبطه لـ 0 ليتحكم كل View في الـ padding الخاص به
    ...
    tools_section = Tools()
    search_field = ft.TextField(hint_text="ابحث عن اسم المشترك...", hint_style=ft.TextStyle(color=ft.colors.GREY_600), prefix_icon=ft.icons.SEARCH, bgcolor=ft.colors.GREY_900, color=ft.colors.WHITE, border=ft.InputBorder.NONE, border_radius=20, content_padding=20, on_change=lambda e: render_data(all_data, e.control.value))

    def render_data(data, query=""):
        members_list.controls.clear()
        total, paid, pending, retracted_count, total_balance, status_text = 0, 0, 0, 0, 0, ""
        if data:
            for key, val in data.items():
                ...
                if val.get("status") == "دفع":
                	if val.get("to") > 0:
                		status_text = f"مقدم {val.get('to')} جمعة"
                	else:
                		status_text = f"تم الدفع"
                	paid += 1
                else:
...
subtitle= ft.Text(status_text, color=ft.colors.GREY_500, size=13), trailing=ft.Icon(ft.icons.CHECK_CIRCLE if val.get("status") == "دفع" else ft.icons.CANCEL, color=ft.colors.LIME_600 if val.get("status") == "دفع" else "#e56328")))
    ...
    def route_change(route):
        page.views.clear()
        header = ft.Container(bgcolor=ft.colors.GREY_900, padding=15, border_radius=10, content=ft.Row(alignment=ft.MainAxisAlignment.SPACE_BETWEEN, controls=[ft.Text(f"الجمعات {get_fridays_count()}", size=14, color=ft.colors.GREY_400), total_balance_text, total_retracted, ft.IconButton(ft.icons.NOTIFICATIONS, icon_color=ft.colors.GREY_600, on_click=lambda _: page.go("/notifications"))]))
        
        # الرئيسية
        page.views.append(ft.View("/", bgcolor=ft.colors.BLACK, padding=ft.padding.only(top=40, left=15, right=15), controls=[ft.Stack(expand=True, controls=[ft.Column([header, tools_section, search_field, members_list], spacing=5), loading_overlay])], floating_action_button=ft.FloatingActionButton(icon=ft.icons.REFRESH, bgcolor=ft.colors.ORANGE_400, on_click=load_data, width=45, height=45)))
        
        # صفحة الإشعارات
        if page.route == "/notifications":
            notif_list = ft.ListView(expand=True)
            if os.path.exists(NOTIFICATIONS_FILE):
                with open(NOTIFICATIONS_FILE, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    for item in data.get("notifications", []):
                        notif_list.controls.append(ft.ListTile(title=ft.Text(item.get("title", ""), color=ft.colors.WHITE), subtitle=ft.Text(f"التاريخ: {item.get('date', '')} | المبلغ: {item.get('amount', '')}", color=ft.colors.GREY_400)))
            
            page.views.append(ft.View("/notifications", bgcolor=ft.colors.BLACK, padding=ft.padding.only(top=40), controls=[
                ft.AppBar(
                    title=ft.Text("الإشعارات"), 
                    bgcolor=ft.colors.GREY_900,
                    leading=ft.IconButton(icon=ft.icons.ARROW_BACK, on_click=lambda _: page.go("/"))
                ),
                notif_list
            ], floating_action_button=ft.FloatingActionButton(icon=ft.icons.REFRESH, bgcolor=ft.colors.ORANGE_400, on_click=lambda e: page.update())))
        page.update()
