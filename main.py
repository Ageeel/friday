...
def get_total_amounts(type):
    total_amount = 0
    notif_path = os.path.join(BASE_DIR, "notifications.json")
...
def save_to_cache(data):
    """حفظ مباشر وآمن للكاش لتجنب مشاكل المرة الأولى"""
    if not data:
        return
    try:
        with open(CACHE_FILE, "w", encoding="utf-8") as f:
            ...
            f.flush()
            os.fsync(f.fileno())  # إجبار النظام على الكتابة القرصية فوراً
...
def main(page: ft.Page):
    ...
    # --- المكونات والإنشاء ---
    all_data = load_from_cache()  # قراءة الكاش فور فتح التطبيق لأول مرة
    view_mode = "active"  # "active" للمشتركين, "retracted" للمنسحبين, "pending" للمطالبين
    ...
    # --- تحسين آلية الحفظ والتحميل لمنع مشكلة المرة الأولى ---
    def load_data(e=None):
        ...
        def fetch_data():
            ...
            # 1. عرض البيانات المخزنة مسبقاً فوراً إن وجدت
            cached = load_from_cache()
            if cached:
                all_data = cached
                render_data(all_data, search_field.value)

            # 2. التحديث من خادم Firebase
            try:
                res = requests.get(DB_URL, timeout=8)
                if res.status_code == 200 and res.json():
                    all_data = res.json()
                    # حفظ مباشر ومؤكد في ملف الكاش
                    save_to_cache(all_data)
            except Exception as err:
                print("Network Exception, using existing cached data:", err)

            # 3. إغلاق شاشة التحميل وتحديث العرض النهائي
    ...
    # عند تشغيل التطبيق لأول مرة: رندر الكاش المحلي فوراً قبل جلب البيانات
    if all_data:
        render_data(all_data, search_field.value)
