...
def main(page: ft.Page):
    ...
    total_balance_text = ft.Text("0", size=16, weight="bold", color=ft.colors.WHITE)
    ...
    def render_data(data, query=""):
        ...
        if data:
            for key, val in data.items():
                ...
                members_list.controls.append(ft.ListTile(leading=ft.CircleAvatar(content=ft.Icon(ft.icons.PERSON, color= ft.colors.ORANGE_600 if val.get("m") == 1 else ft.colors.ORANGE_900), bgcolor=ft.colors.GREY_900), title=ft.Text(val.get("name", "مجهول"), color=ft.colors.GREY_200), subtitle=ft.Text(status_text, color=ft.colors.GREY_500, size=13), trailing=ft.Icon(icon, color=icon_color)))
        
        tools_section.update_stats(total, paid, pending)
        total_balance_text.value = str(total_balance - total_withdrawals)
    ...
    onboarding_container = ft.Container(
        visible=False, bgcolor=ft.colors.BLACK, alignment=ft.alignment.center, expand=True,
        content=ft.Column(
            alignment=ft.MainAxisAlignment.CENTER,
            horizontal_alignment=ft.CrossAxisAlignment.CENTER,
            controls=[
                ft.Icon(ft.icons.WALLET, size=45, color=ft.colors.ORANGE_400), 
                ft.Text("نحول مدخراتنا الأسبوعية لفرصة إستثمارية حقيقية. يلا إشترك معانا والتزم بـ 1,000 جنيه كل يوم جمعة، كل دا عشان نسوي راس مال ، ونخطط لمشروع يخدمنا في المستقبل ويرفع مكانة الأسرة", size=22, color="white", text_align=ft.TextAlign.CENTER),
    ...
    def route_change(route):
        ...
        header = ft.Card(
            ...
            content=ft.Container(
                padding=20,
                gradient=ft.LinearGradient(begin=ft.alignment.top_left, end=ft.alignment.bottom_right, colors=[ft.colors.GREY_900, ft.colors.BLACK]),
                content=ft.Column(spacing=10, controls=[
                    ft.Row(alignment=ft.MainAxisAlignment.SPACE_BETWEEN, controls=[
                        ft.Text("كل جمعة", size=20, weight="bold", color=ft.colors.ORANGE_400), 
                        build_notification_icon(page)
                    ]),
                    ft.Row(alignment=ft.MainAxisAlignment.SPACE_EVENLY, controls=[
                        ft.Column([ft.Text("الجمعات", size=10, color=ft.colors.GREY_300), ft.Text(str(total_fridays), size=16, weight="bold", color=ft.colors.GREY_300)], horizontal_alignment=ft.CrossAxisAlignment.CENTER),
                        ft.VerticalDivider(), 
                        ft.Column([ft.Text("الرصيد", size=10, color=ft.colors.GREY_300), total_balance_text], horizontal_alignment=ft.CrossAxisAlignment.CENTER),
                        ft.VerticalDivider(), 
                        ft.Column([ft.Text("المنسحبين", size=10, color=ft.colors.GREY_300), total_retracted], horizontal_alignment=ft.CrossAxisAlignment.CENTER),
                    ])
                ])
            )
        )
        
        main_content = ft.View("/", bgcolor=ft.colors.BLACK, padding=ft.padding.only(top=50, left=15, right=15), controls=[
            ft.Stack(expand=True, controls=[
                ft.Column([header, tools_section, search_field, members_list], spacing=5), 
                loading_overlay, 
                onboarding_container
            ])
        ], floating_action_button=refresh_button)
        
        page.views.append(main_content)
