...
def main(page: ft.Page):
    ...
    class Tools(ft.Container):
        ...
        def tool_item(self, title, counter):
            ...
            return ft.Container(expand=True, height=80, bgcolor=ft.colors.GREY_900, border_radius=5, padding=10, content=ft.Column(alignment=ft.MainAxisAlignment.CENTER, horizontal_alignment=ft.CrossAxisAlignment.CENTER, controls=[ft.Text(title, color=color, size=12), counter]),gradient=ft.LinearGradient(begin=ft.alignment.bottom_center, end=ft.alignment.top_center, colors=["#111113", "#111112"])
            )
    ...
    def route_change(route):
        page.views.clear()
        header = ft.Card(margin=ft.margin.all(0), elevation=5, content=ft.Container(padding=20, border_radius=10, gradient=ft.LinearGradient(begin=ft.alignment.bottom_left, end=ft.alignment.top_right, colors=["#110111", "#111212"]), content=ft.Column(spacing=10, controls=[
            ...
            ft.Row(alignment=ft.MainAxisAlignment.SPACE_EVENLY, controls=[
                ...
                ft.VerticalDivider(), 
                ft.Column([ft.Text("المنسحبين", size=14, color=ft.colors.GREY_300), total_retracted], horizontal_alignment=ft.CrossAxisAlignment.CENTER),
            ])
        ])))
