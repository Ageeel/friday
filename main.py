...
def main(page: ft.Page):
    ...
    page.padding = 0
    # --- القراءة المباشرة من كاش التطبيق (Client Storage) ---
        ...
        if data:
            ...
            for key, val in data.items():
                ...
                if view_mode == "retracted":
                    ...
                    members_list.controls.append(
                        ft.ListTile(
                            leading=ft.CircleAvatar(content=ft.IconButton(ft.icons.PERSON, icon_color=p_icon_col, on_click=lambda _: page.launch_url(val.get("contact"))), bgcolor=ft.colors.GREY_900), 
        ...
        def fetch_data():
            ...
            try:
                ...
                if res.status_code == 200 and res.json():
                    all_data = res.json()
                    # حفظ مباشر في تخزين التطبيق الآمن
                    page.client_storage.set("data_cache", all_data)
            except Exception as err:
                print("Fetch error, using client storage:", err)
                # في حال الفشل نعتمد على الكاش المخزن
    ...
    # عند بداية تشغيل التطبيق: عرض الكاش فوراً إن وجد
    if all_data:
        render_data(all_data, search_field.value)
