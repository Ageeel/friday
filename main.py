...
def main(page: ft.Page):
    ...
    # جلب تخزين الصور المؤقت من client_storage
    images_cache = page.client_storage.get("images_cache")
    if not isinstance(images_cache, dict):
        images_cache = {}
    ...
    # دالة لتحميل الصور وتخزينها في الـ Cache بشكل تدريجي في الخلفية
        ...
        def background_worker():
            ...
            for key, val in data.items():
                if isinstance(val, dict):
                    img_url = val.get("img")
                    if img_url and img_url.startswith("http") and key not in images_cache:
                        try:
                            img_res = requests.get(img_url, timeout=5)
                            if img_res.status_code == 200:
                                encoded_img = base64.b64encode(img_res.content).decode("utf-8")
                                images_cache[key] = f"data:image/jpeg;base64,{encoded_img}"
                                updated = True
                        except Exception:
                            pass
            if updated:
                try:
                    page.client_storage.set("images_cache", images_cache)
                    render_data(all_data, search_field.value)
                except Exception:
                    pass
        ...
        if data:
            ...
            for key, val in data.items():
                ...
                if view_mode == "retracted":
                    ...
                    # استخدام الصورة المخزنة في الـ Cache المحلي إن وجدت، وإلا فالرابط المباشر
                    profile_img = images_cache.get(key) or val.get("img", "profile.png")

                    members_list.controls.append(
                        ft.ListTile(
                            leading=ft.CircleAvatar(
                                foreground_image_url=profile_img,
                                bgcolor=ft.colors.GREY_900
                            ), 
    ...
    def load_data(e=None):
        ...
        def fetch_data():
            ...
            # بدء تحميل وتخزين الصور في الـ Cache بالخلفية بعد عرض البيانات
